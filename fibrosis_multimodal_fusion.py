...
class Config:
    ...
    output_dir: str = "outputs_fibrosis_multimodal"
    seed: int = 42
    n_folds: int = 5
...
# cohort (no patient- or sex-specific reference ranges are available).
ULN_AST = 40.0  # U/L, used in APRI
ULN_GGT = 40.0  # U/L, used in GPR
...
def load_cohort(tabular_csv: str) -> pd.DataFrame:
    ...
    binary fibrosis target (F0 -> 0, F1-F2 -> 1)."""
...
def add_fibrosis_indices(df: pd.DataFrame) -> pd.DataFrame:
    """Append the five fibrosis-specific derived features. Applied to
    ...
    # AST/ALT ratio (shared with the steatosis feature set).
    df["AST_ALT_ratio"] = ast / alt

    # FIB-4 = Age * AST / (Platelets * sqrt(ALT)).
    df["FIB4"] = (age * ast) / (platelets * np.sqrt(alt))

    # APRI = ((AST / ULN_AST) / Platelets) * 100, ULN_AST = 40 U/L.
    df["APRI"] = ((ast / ULN_AST) / platelets) * 100.0

    # GPR = ((GGT / 40) / Platelets) * 100.
    df["GPR"] = ((ggt / ULN_GGT) / platelets) * 100.0

    # Forns = 7.811 - 3.131 ln(Platelets) + 0.781 ln(GGT)
...
def train_one_fold(fold, train_pids, test_pids, cohort_df, frame_df,
    ...
    label_by_pid = cohort_df.set_index("patient_id")["fibrosis_binary"]
...
def run_cross_validation(cohort_df, frame_df, cfg, device, out_dir):
    ...
    labels = (
        cohort_df[["patient_id", "fibrosis_binary"]]
        .rename(columns={"fibrosis_binary": "label"})
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Multimodal gated residual fusion — fibrosis "
                    "(F0 vs F1-F2)."
    )
