...
class Config:
    ...
    output_dir: str = "outputs_steatosis_image"
    seed: int = 42
    n_folds: int = 5
...
def load_patient_labels(tabular_csv: str) -> pd.DataFrame:
    """Binary steatosis label per patient: S0-S1 -> 0, S2-S3 -> 1."""
    ...
    labels = pd.DataFrame({
        ...
        "label": (stage.astype(int) >= 2).astype(int),
    })
    ...
    print(f"Patients: {len(labels)} | Class 0 (S0-S1): {counts[0]}, "
...
def run_cross_validation(labels, frame_df, cfg, device, out_dir):
    ...
    seed give the same folds as the other configuration scripts."""
    skf = StratifiedKFold(
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Image-only EfficientNet-B0 — steatosis (S0-S1 vs S2-S3)."
    )
