...
class Config:
    ...
    output_dir: str = "outputs_steatosis_tabular"
    seed: int = 42
    n_folds: int = 5
...
# Task constants 
TASK_NAME = "Steatosis"
LABEL_COL = "steatosis_binary"
STAGE_COL = "Steatosis stage"
...
def load_cohort(cfg: Config) -> pd.DataFrame:
    """Load BEHSOF clinical records and build the binary steatosis target."""
    ...
    # Binary target: S0-S1 -> 0 ("none-mild"), S2-S3 -> 1 ("moderate-severe").
    df[STAGE_COL] = pd.to_numeric(df[STAGE_COL], errors="coerce")
    assert df[STAGE_COL].notna().all(), "Invalid steatosis stage values."
    df[LABEL_COL] = (df[STAGE_COL].astype(int) >= 2).astype(int)
...
# ---------------------------------------------------------------------------
# 2. Engineered steatosis indices 
# ---------------------------------------------------------------------------


def add_steatosis_indices(df: pd.DataFrame) -> pd.DataFrame:
    """Append the five steatosis-specific derived features.
    ...
    zero/negative flooring: the paper states all log inputs were strictly
    positive in this cohort, so positivity is asserted instead.
    """
    df = df.copy()
    ...
    # Eq. (1): AST/ALT ratio (shared with the fibrosis feature set).
    df["AST_ALT_ratio"] = ast / alt

    # Eq. (2): TyG = ln(TG[mg/dL] * FBS[mg/dL] / 2).
    df["TyG_index"] = np.log(tg * fbs / 2.0)

    # Eq. (3): HSI = 8*(ALT/AST) + BMI + 2*I(Female) + 2*I(Diabetes).
    # BEHSOF carries no diabetes field; the indicator is derived from fasting
    # blood sugar >= 126 mg/dL (ADA fasting diagnostic criterion).
    female = (df["sex"] == 1).astype(float)
    diabetes = (fbs >= 126.0).astype(float)
    df["HSI"] = 8.0 * (alt / ast) + bmi + 2.0 * female + 2.0 * diabetes

    # Eq. (4): FLI = expit(L) * 100,
    ...
    # Eq. (5): LAP with TG converted to mmol/L (TG[mg/dL] / 88.5);
...
def make_model(seed: int) -> LogisticRegression:
    """Tabular Logistic Regression classifier (paper Section III.C.1)."""
...
# ---------------------------------------------------------------------------
# 4. Metrics and threshold selection (paper Section V.A)
# ---------------------------------------------------------------------------
...
def run_shap(
    ...
    # --- Single-patient waterfall (default: BEH01121, paper Section V.E) ---
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(
        description="Tabular LR baseline — steatosis (S0-S1 vs S2-S3)."
    )
